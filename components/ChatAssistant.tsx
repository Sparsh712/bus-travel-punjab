...
// A new type to represent a specific journey taken by the user
interface Journey extends Route {
    journeyFare: number;
    journeyStops: string[];
}
...
interface ChatMessage {
  ...
  text: string;
  routeSuggestion?: Journey; // Now suggests a specific journey
}

// --- CORE DATA SOURCE ---
// UPDATED: Added a `fare_per_stop` to enable dynamic fare calculation.
const busDataSource = [
    { id: 'bus-001', route: 'Model Town to Liberty Market', fare_per_stop: 10, stops: [ { stop_name: 'Model Town Park', eta: 2 }, { stop_name: 'Kalma Chowk', eta: 8 }, { stop_name: 'Centre Point', eta: 15 }, { stop_name: 'Liberty Market', eta: 20 }, ], },
    { id: 'bus-002', route: 'Railway Station to DHA', fare_per_stop: 8, stops: [ { stop_name: 'Railway Station', eta: 3 }, { stop_name: 'Garhi Shahu', eta: 10 }, { stop_name: 'Dharampura', eta: 18 }, { stop_name: 'LUMS', eta: 25 }, { stop_name: 'Y-Block Market', eta: 30 }, ], },
    { id: 'bus-003', route: 'Thokar Niaz Baig to Anarkali', fare_per_stop: 9, stops: [ { stop_name: 'Thokar Niaz Baig', eta: 5 }, { stop_name: 'Multan Chungi', eta: 12 }, { stop_name: 'Chauburji', eta: 20 }, { stop_name: 'MAO College', eta: 25 }, { stop_name: 'Anarkali', eta: 30 }, ], },
    { id: 'bus-004', route: 'Gajumata to Azadi Chowk', fare_per_stop: 10, stops: [ { stop_name: 'Gajumata', eta: 5 }, { stop_name: 'Nishtar Colony', eta: 15}, { stop_name: 'Kalma Chowk', eta: 25 }, { stop_name: 'Ichhra', eta: 35 }, { stop_name: 'Data Darbar', eta: 45 }, { stop_name: 'Azadi Chowk', eta: 55 }, ], },
    { id: 'bus-005', route: 'Green Town to Shahdara', fare_per_stop: 9, stops: [ { stop_name: 'Green Town', eta: 5 }, { stop_name: 'Model Town', eta: 15 }, { stop_name: 'Canal Road', eta: 25 }, { stop_name: 'The Mall', eta: 35 }, { stop_name: 'Shahdara', eta: 50 }, ], },
    { id: 'bus-006', route: 'Johar Town to Mughalpura', fare_per_stop: 8, stops: [ { stop_name: 'Johar Town', eta: 5 }, { stop_name: 'Shaukat Khanum', eta: 10 }, { stop_name: 'Jinnah Hospital', eta: 20 }, { stop_name: 'Ferozepur Road', eta: 30 }, { stop_name: 'Mughalpura', eta: 45 }, ], },
    { id: 'bus-007', route: 'Cantt to Samanabad', fare_per_stop: 10, stops: [ { stop_name: 'Lahore Cantt', eta: 5 }, { stop_name: 'Fortress Stadium', eta: 10 }, { stop_name: 'Saddar', eta: 15 }, { stop_name: 'Jail Road', eta: 25 }, { stop_name: 'Samanabad', eta: 35 }, ], },
    { id: 'bus-008', route: 'Walled City to Gulberg', fare_per_stop: 7, stops: [ { stop_name: 'Delhi Gate', eta: 5 }, { stop_name: 'Anarkali', eta: 10 }, { stop_name: 'Civil Lines', eta: 18 }, { stop_name: 'Shadman', eta: 25 }, { stop_name: 'Gulberg Main Market', eta: 35 }, ], }
];

// This now includes the fare_per_stop for calculations
const routes = busDataSource.map(bus => ({ id: bus.id, name: bus.route, stops: bus.stops.map(stop => stop.stop_name), duration: `${bus.stops[bus.stops.length - 1]?.eta || 0} mins`, fare_per_stop: bus.fare_per_stop }));
...
// Added SHOW_COMPLETE_ROUTE topic
...
const classifyUserIntent = async (query: string, context: Journey | null): Promise<{ intent: Intent; topic?: AppNavigationTopic }> => {
    await new Promise(res => setTimeout(res, 300));
    const lowerCaseQuery = query.toLowerCase();

    // --- MEMORY CHECK (IMPROVED) ---
    if (context) {
    ...
    if (travelKeywords.some(keyword => lowerCaseQuery.includes(keyword))) {
        // If it's a general fare question without context, treat as App Navigation
...
// 2. Fuzzy Text Matching Utility (Levenshtein Distance)
...
// 3. Smart Route Finding Logic
const getSmartTravelAdvice = async (query: string): Promise<{ summary: string; journey?: Journey }> => {
    ...
    if (fromLocation && toLocation) {
        const findBestMatch = (location: string) => {
            let bestMatch = ''; let minDistance = 2;
            allStops.forEach(stop => {
                const distance = calculateLevenshteinDistance(location, stop.toLowerCase());
        ...
        if (correctedFrom && correctedTo && correctedFrom.toLowerCase() !== correctedTo.toLowerCase()) {
            const foundRoute = routes.find(route => {
                ...
                const fromIndex = stopsLower.indexOf(correctedFrom.toLowerCase());
                const toIndex = stopsLower.indexOf(correctedTo.toLowerCase());
                return fromIndex !== -1 && toIndex !== -1 && fromIndex < toIndex;
            });
            if (foundRoute) {
                // --- FARE CALCULATION LOGIC ---
                const fromIndex = foundRoute.stops.map(s => s.toLowerCase()).indexOf(correctedFrom.toLowerCase());
                const toIndex = foundRoute.stops.map(s => s.toLowerCase()).indexOf(correctedTo.toLowerCase());
                const stopsCount = toIndex - fromIndex;
                const calculatedFare = stopsCount * foundRoute.fare_per_stop;

                const journey: Journey = {
                    id: foundRoute.id,
                    name: `${correctedFrom} to ${correctedTo}`,
                    stops: foundRoute.stops,
                    journeyStops: foundRoute.stops.slice(fromIndex, toIndex + 1),
                    duration: 'N/A',
                    fare: busDataSource.find(b => b.id === foundRoute.id)!.stops[busDataSource.find(b => b.id === foundRoute.id)!.stops.length-1].eta,
                    journeyFare: calculatedFare
                };
...
const RouteResultCard: React.FC<{ route: Journey }> = ({ route }) => (
    <div className="border rounded-lg p-3 bg-gray-100 dark:bg-gray-600">
        <h3 className="font-bold text-md text-orange-600 dark:text-orange-400">{route.name}</h3>
        {/* Now shows only the relevant stops for the journey */}
        <p className="text-sm mt-1"><span className="font-semibold">Stops:</span> {route.journeyStops.join(' → ')}</p>
        {/* Shows the specific calculated fare */}
        <p className="text-sm mt-1"><span className="font-semibold">Fare:</span> ₹{route.journeyFare}</p>
    </div>
);
...
const ChatAssistant: React.FC = () => {
  ...
  const [lastRouteContext, setLastRouteContext] = useState<Journey | null>(null);
    ...
    try {
      ...
      switch (classified.intent) {
        case Intent.CONTEXTUAL_QUERY:
            if (lastRouteContext) {
                if (classified.topic === AppNavigationTopic.FARE_INFO) {
                    aiMessage = { author: MessageAuthor.AI, text: `The fare for your trip from ${lastRouteContext.journeyStops[0]} to ${lastRouteContext.journeyStops[lastRouteContext.journeyStops.length - 1]} is ₹${lastRouteContext.journeyFare}.` };
                } else if (classified.topic === AppNavigationTopic.SHOW_COMPLETE_ROUTE) {
                    aiMessage = { author: MessageAuthor.AI, text: `Sure! Here is the complete list of stops for the '${lastRouteContext.name}' route:\n\n${lastRouteContext.stops.join(' → ')}` };
                } else {
        ...
        case Intent.TRAVEL_QUERY:
          ...
          if (aiResponse.journey) {
            setLastRouteContext(aiResponse.journey);
          } else {
            setLastRouteContext(null);
          }
    ...
    return (
      <div className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
        ...
        <div className={`max-w-md p-4 rounded-xl ${isUser ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-bl-none'}`}>
          ...
          {msg.routeSuggestion && ( <div className="mt-4"><p className="text-sm font-semibold mb-2">Here's the journey I found:</p><RouteResultCard route={msg.routeSuggestion} /></div> )}
        </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-4xl mx-auto flex flex-col h-[75vh] animate-fade-in">
      <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center">
        <h2 className="text-xl font-bold text-center flex-1">AI Travel Assistant</h2>
      </div>
